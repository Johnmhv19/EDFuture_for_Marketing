...
function Fact({ label, value }) {
  // The structured Date row always renders (with "TBD" when both
  // startDate / endDate are null). Other rows may be missing.
  const missing = value === null || value === undefined || value === '';
  return (
    <div>
      <div className="text-xs uppercase tracking-wide text-gray-500 font-bold">{label}</div>
      <div className={missing ? 'text-gray-500 italic' : 'text-gray-900'}>
        {missing ? '—' : value}
      </div>
    </div>
  );
